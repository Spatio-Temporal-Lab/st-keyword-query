package com.github.pbf.base;

import com.github.pbf.util.MurmurHash3;
 ...
 *
 */
public class BasicBloomFilter implements Serializable
{
	private BitSet bs;
	private int bitNum;
	private int hashNum;
	...
	public BasicBloomFilter(int bitNum, int[] seeds, int hashNum)
	{
		this.seeds = seeds;
		this.bitNum = bitNum;
...
//		this.hashNum = 4;
	...
	public void insert(byte[] element)
	{
		if (seeds != null) {
			for (int i = 0; i < hashNum; ++i) {
				int finalI = i;
				new Thread(() -> {
					int hash = MurmurHash3.murmurhash3_x86_32(element, 0, element.length, seeds[finalI]);
	...
	public boolean query(byte[] element)
	{
		...
		for (int i = 0; i < hashNum; i++)
		{
			int hash;
			if (seeds == null) {
				hash = MurmurHash3.murmurhash3_x86_32(element, 0,
						element.length, i);
			} else {
				hash = MurmurHash3.murmurhash3_x86_32(element, 0,
						element.length, seeds[i]);
			}
			int index = getIndex(hash);
