package com.github.pbf.util;

import com.github.pbf.CommonConstants;
import com.github.pbf.base.impl.TemporalRangeBloomFilterV2;
import org.apache.commons.math3.analysis.solvers.BisectionSolver;

import java.util.BitSet;

public class PbfUtil2
{
	...
	public static int[] getOptimizedMForBeta1(int m, int[] d, int[] f, int queryLength)
	{
		...
		else
		{
			...
			double lamda = solver.solve(20000, lamdaFunc, -1, 0);
			...
			for (int i = 0; i < optimizedM.length; i++)
			{
				...
				else
				{
					optimizedM[i] = (int) ((double) d[i] / INVARIANT
							* Math.log(1.0D - (double) f[i] * PbfUtil2.INVARIANT / lamda / d[i]));
				}
			}
		}
		return optimizedM;
...
//						* Math.log(1.0D - (double) f[i] * PbfUtil2.INVARIANT / x / d[i]);
//			}
//		}
//		result = result - (double) m * PbfUtil2.INVARIANT;
//		return result;
//	}
//}