...
public class LevelSTKeyGenerator extends SpatialFirstSTKeyGenerator {
    int level;
    BloomFilter<byte[]>[] bloomFilters;

    public LevelSTKeyGenerator(int level, int maxSize) {
        this.level = level;
    ...
    public LevelSTKeyGenerator(int level, BloomFilter<byte[]>[] bloomFilters) {
        this.level = level;
    ...
    public byte[] getKeyInLevel(byte[] key, int level) {
        ...
        long t = ByteUtil.toLong(Arrays.copyOfRange(key, SPATIAL_BYTE_COUNT, SPATIAL_BYTE_COUNT + TIME_BYTE_COUNT));
        return ByteUtil.concat(
                ByteUtil.getKByte(s >> ((this.level - 1 - level) << 1), SPATIAL_BYTE_COUNT),
                ByteUtil.getKByte(t >> (this.level - 1 - level), TIME_BYTE_COUNT)
        );
    }
