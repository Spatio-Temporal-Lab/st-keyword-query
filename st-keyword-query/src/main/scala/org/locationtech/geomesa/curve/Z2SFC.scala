...
class Z2SFC(precision: Int) extends SpaceFillingCurve {
  ...
  import SpaceFillingCurve.FullPrecision
   def toRanges(x: (Double, Double), y: (Double, Double)): Seq[ZIndexRange] = {
    val indexRanges = ranges(Seq((x._1, y._1, x._2, y._2)), FullPrecision, None)
    indexRanges.map(indexRange => new ZIndexRange(indexRange.lower, indexRange.upper))
  }
}
